...
Other available helpers include ``is_public_addr``, ``is_ip_unspecified``,
``is_ip6_link_local``, ``is_thin_waist``, and ``is_nat64_ipv4_converted_ipv6_addr``.
These operate on ``Multiaddr`` objects; ``is_wildcard`` / ``is_link_local_ip``
remain available for raw IP strings.
