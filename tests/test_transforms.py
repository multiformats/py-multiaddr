...
from multiaddr.transforms import (
    bytes_iter,
    bytes_to_string,
    size_for_addr,
    string_iter,
...
def test_string_iter_stacked_unix_path():
    addr = "/ip4/1.2.3.4/tcp/80/unix/a/b/c"
    parts = list(string_iter(addr))
    assert [(proto.name, value) for proto, _, value in parts] == [
        ("ip4", "1.2.3.4"),
        ("tcp", "80"),
        ("unix", "a/b/c"),
    ]
    assert string_to_bytes(addr) == Multiaddr(addr).to_bytes()
